...
@($cacheDir, $resourcesDir, $binariesDir, $mitmTargetDir, $nodeTargetDir, $scriptsTargetDir, $schemaTargetDir) | ForEach-Object {
    if (-Not (Test-Path $_)) {
...
# -------------------------------------------------------------------
Write-Host "`n[1/6] Compiling DAWG Go Engine..." -ForegroundColor Yellow
Set-Location -Path $engineDir
$engineTargetExe = Join-Path $binariesDir "dawg.exe"
go build -o $engineTargetExe ./cmd/dawg
...
# -------------------------------------------------------------------
Write-Host "`n[2/6] Staging Standalone mitmdump (v$MitmVersion)..." -ForegroundColor Yellow
...
if (-Not (Test-Path $mitmExe) -or -Not $SkipDownload) {
    ...
    if (-Not (Test-Path $mitmZip) -and -Not (Test-Path $mitmExe)) {
...
# -------------------------------------------------------------------
Write-Host "`n[3/6] Staging Portable Node.js (v$NodeVersion)..." -ForegroundColor Yellow
...
if (-Not (Test-Path $nodeExe) -or -Not $SkipDownload) {
    ...
    if (-Not (Test-Path $nodeZip) -and -Not (Test-Path $nodeExe)) {
...
# -------------------------------------------------------------------
# 4. Provision Playwright & Node Modules
# -------------------------------------------------------------------
Write-Host "`n[4/6] Provisioning Playwright & rrweb dependencies..." -ForegroundColor Yellow
...
if (Test-Path $engineNodeModules) {
    ...
    Copy-Item -Path (Join-Path $engineDir "package.json") -Destination (Join-Path $resourcesDir "package.json") -Force
...
# -------------------------------------------------------------------
Write-Host "`n[5/6] Copying engine scripts and JSON schema/policies..." -ForegroundColor Yellow
Copy-Item -Path "$engineDir\scripts\*" -Destination $scriptsTargetDir -Recurse -Force
Copy-Item -Path "$schemaSourceDir\*" -Destination $schemaTargetDir -Recurse -Force
Write-Host "  -> Scripts & Schema staged into $resourcesDir" -ForegroundColor Green

# -------------------------------------------------------------------
# 6. Verify Staged Bundle via DAWG Doctor
# -------------------------------------------------------------------
Write-Host "`n[6/6] Verifying Staged Monolithic Bundle with 'dawg doctor'..." -ForegroundColor Yellow
$env:DAWG_RESOURCES_DIR = $resourcesDir
& $engineTargetExe doctor
if ($LASTEXITCODE -ne 0) {
    Write-Warning "DAWG doctor reported degraded status for staged bundle."
} else {
...
if (-Not $SkipTauri) {
    ...
    Write-Host "Run 'npm run tauri build' inside 'desktop/' to compile installer." -ForegroundColor DarkGray
}
