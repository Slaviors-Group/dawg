---
layout: home
title: "DAWG - Digs Any Web-app Glitch"
titleTemplate: "Desktop Capture and Portable .dawg Artifacts"
description: "DAWG 0.3.5-middlechild pairs a Tauri desktop app with a Chromium extension to capture, sanitize, catalog, import, export, and replay portable OCI-based .dawg artifacts."
head:
  - [meta, { name: "theme-color", content: "#6B21D9" }]
  - [meta, { property: "og:type", content: "website" }]
  - [meta, { property: "og:title", content: "DAWG - Digs Any Web-app Glitch" }]
  - [meta, { property: "og:description", content: "Capture one browser tab, sanitize it, and share the validated OCI artifact as a portable .dawg archive." }]
  - [meta, { property: "og:url", content: "https://dawg.slaviors.id" }]
  - [meta, { property: "og:image", content: "https://dawg.slaviors.id/og-image.png" }]
  - [meta, { property: "og:image:type", content: "image/png" }]
  - [meta, { property: "og:image:width", content: "1200" }]
  - [meta, { property: "og:image:height", content: "630" }]
  - [meta, { property: "og:image:alt", content: "DAWG — Digs Any Web-app Glitch" }]
  - [meta, { name: "twitter:card", content: "summary_large_image" }]
  - [meta, { name: "twitter:title", content: "DAWG - Digs Any Web-app Glitch" }]
  - [meta, { name: "twitter:description", content: "Capture, sanitize, catalog, import, export, and replay portable .dawg artifacts." }]
  - [meta, { name: "twitter:image", content: "https://dawg.slaviors.id/og-image.png" }]
  - [meta, { name: "twitter:image:alt", content: "DAWG — Digs Any Web-app Glitch" }]
home: true
---
